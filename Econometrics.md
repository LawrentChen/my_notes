...
1. 线性假定，真实DGP线性于参数
2. 严外生性 $E(\varepsilon_i|X)=0$。这意味着可以推出扰动项和所有解释变量均不相关，$Cov(\varepsilon_i,x_{jk})=0，\forall j,k$。另外用期望迭代即可有 $E(\varepsilon_i)=0$
...
注意：要区分条件方差和无条件方差。**计量中的异方差都是条件异方差，而不是无条件异方差**。即便在大样本假定下仍要求样本数据为严平稳过程，严平稳自然方差就不变，这个要求是指

- 无条件方差 $Var(\varepsilon_i)=\sigma^2$ 不随 $i$ 而改变
- 条件方差的函数形式也完全一致，但可以含有解释变量 $x$，比如 $Var(\varepsilon_i|x_1,\cdots,x_n)=x_i^2$，这样就允许了存在条件异方差。
...
1. 后果
   ...
   2. 如果被遗漏变量和解释变量相关，那么扰动项也和解释变量相关。出现内生性问题，大样本下 OLS 不一致。高估还是低估的方向在两个变量的情况下比较容易推测，但变量数量一多就难了
...
为了缓和内生性（解释变量和扰动项相关，违背前定变量或者叫同期外生的假设）

内生性来源可以是：
1. 遗漏变量
2. 联立方程/双向因果/同时性
3. 测量误差：解释变量的测量误差 $x=x^*+e$，带来的是一种“衰减偏误”。如果是被解释变量的测量误差，只要这个测量误差与解释变量不相关，那 OLS 依然一致
4. 选择性偏误：如随机试验退出、非平衡面板数据丢失等自选择问题



### 工具变量条件

1. 相关性：与内生解释变量相关，$Cov(z_t,x_t)\neq0$
2. 外生性：与扰动项不相关，$Cov(z_t,u_t)=0$



工具变量估计量
$$
\hat{\beta}_{IV}=\dfrac{\widehat{Cov(x_t,z_t)}}{\widehat{Cov(y_t,z_t)}}\overset{p}{\rightarrow}\dfrac{Cov(x_t,z_t)}{Cov(y_t,z_t)}=\beta
$$
...
原理是把内生变量分为两个部分，一部分是由工具变量造成的外生部分，另一部分是与扰动项相关的残余部分。被解释变量只对外生部分进行回归即可得到一致估计。

1. 第一阶段，内生解释变量对**所有外生变量**回归。注意不只是对工具变量回归，还要对原模型中的外生变量回归，可以理解为外生变量作为自己的工具变量
2. 第二阶段，用第一阶段回归的拟合值替代原内生解释变量，放回原模型再做回归。注意不要丢了原有的外生解释变量
3. 阶条件：工具变量个数不少于内生解释变量个数
   1. 不可识别：工具变量个数少于内生解释变量个数，因为这样第一阶段回归时就会把较多的内生解释变量表示为较少工具变量的线性组合，从而出现严格多重共线性
   2. 恰足识别：工具变量个数等于内生解释变量个数
   3. 过度识别：工具变量个数大于内生解释变量个数

**注意：原模型的残差和第二阶段回归的残差不相等。不要手动做两次回归，实操时直接用封装好的命令**



### 弱工具变量

如果工具变量和内生解释变量只是微弱地相关，那么工具变量法估计的系数 $\hat{\beta}_{IV}$ 方差会很大。直观上来说是用了一个只含有极少与原来内生解释变量有关信息的新变量来进行估计，所以很不准确。小样本真实分布也会离大样本渐进正态相去甚远，所以大样本统计推断失效。

弱工具变量甚至可能不如 OLS

**弱工具变量检验**，必须要报告。但是其显著性却只有一个经验法则，而且要随样本量、方法等等区别，实操时直接用封装好的命令，别自己套经验。注意一般都采用异方差版本的结果以支持稳健性

如果出现弱工具变量问题，可能的解决方法：

- 寻找更强的工作变量。有一个套路叫 **Bartik**，可以用原来的弱工具变量和其他一些东西组成**交互项**一起进入回归，比如说全国性的时间趋势和内部地方性的份额结构
- 使用对弱工具变量更不敏感的有限信息最大似然估计（LIML）



### 过度识别检验

检验的名字有点问题：检验工具变量的**外生性**，**不是**用来检验是不是过度识别的（变量数量多少一眼就看出来了）。原假设 $H_0$：所有工具变量都是外生的

- 在恰好识别的情况下，没有办法检验工具变量的外生性
- 只有在过度识别的情况下，才可以进行过度识别检验，判断是否至少有一个工具变量是外生的。
  - **Sargan 检验**
  - Hansan'J 检验

**现在基本不太用了，因为如果有问题也不知道该怎么办。不同结果之间，并不知道哪一个结果收敛到真实值**



### 内生性检验

检验解释变量是否存在内生性。原假设 $H_0$：所有解释变量都为外生变量

- **豪斯曼检验**：需要先找好 IV，所以如果文章卖点就是 IV 的话，最好是要显著，否则前期找的 IV 等于白找
- DWH：异方差下的豪斯曼检验
...
因为个体效应 $u_i$ 和某个解释变量相关，所以 OLS 不一致。需要想办法消掉这个 $u_i$ 项。对给定个体 $i$，在 $\eqref{个体效应模型}$ 式两边对时间取平均
$$
...
$\eqref{个体效应模型}$ 式中，因为 $u_i$ 与所有的解释变量 $(\mathbf{x}_{it},\mathbf{z}_i)$ 均不相关，所以 OLS 是一致的。但是扰动项由 $(u_i+\varepsilon_{it})$ 组成，就不是球状扰动了。虽然大样本下不是球状扰动也行，但问题是还存在自相关。

即便假设不同个体之间的扰动项 $u_i$ 互不相关，但不同时期的扰动项之间仍然会存在自相关，即：
$$
\begin{align}
Cov(u_i+\varepsilon_{it},u_i+\varepsilon_{is})&=Cov(u_i,u_i)+\underbrace{Cov(u_i,\varepsilon_{is})}_{=0}+\underbrace{Cov(\varepsilon_{it},u_i)}_{=0}+\underbrace{Cov(\varepsilon_{it},\varepsilon_{is})}_{=0}\\
&=Var(u_i^2)=\sigma_u^2\neq 0
\end{align}
$$
这个自相关使得 OLS 不是最有效率的。可以使用**广义最小二乘法（GLS）**进行变换，使得变换后的扰动项不再有自相关



首先定义
$$
\theta\equiv1-\dfrac{\sigma_{\varepsilon}}{(T\sigma_u^2+\sigma^2_{\varepsilon})^{1/2}}
$$
对给定个体 $i$，在 $\eqref{个体效应模型}$ 式两边对时间取平均。再两边同乘 $\theta$，再用原方程减去上面的方程，即可得到**广义离差模型**
$$
y_{it}-\theta\overline{y}_i=(\mathbf{x}_{it}-\theta\overline{\mathbf{x}}_{i})^T\mathbf{\beta}+(1-\theta)\mathbf{z}_i^T\mathbf{\delta}+[(1-\theta)u_i+(\varepsilon_{it}-\theta\overline{\varepsilon}_i)]
$$
这个做法和固定效应组内估计量的做法几乎完全一样，就是中间多乘了一个 $\theta$，**也就是异方差的处理办法**。可以证明，上式扰动项不再有自相关问题。对其进行估计即可得到 GLS 估计量。

然而，这个 $\theta$ 通常未知，还需要先行估计 $\hat{\theta}$。再做真正可行的 FGLS 估计
$$
\hat{\theta}\equiv1-\dfrac{\hat{\sigma}_{\varepsilon}}{(T\hat{\sigma}_u^2+\hat{\sigma}^2_{\varepsilon})^{1/2}}
$$
因为随机效应模型最初形式的 OLS 还是一致的（尽管方差有问题），且其最初形式的扰动项是 $(u_i+\varepsilon_{it})$ ，所以就可以用 OLS 的残差来估计 $(\sigma_u^2+\sigma_{\varepsilon}^2)$。另外，FE 也是一致的，且其扰动项是 $(\varepsilon_{it}-\overline{\varepsilon}_{i})$，所以可以用 FE 的残差来估计 $\sigma_{\varepsilon}^2$。$\hat{\theta}$ 的分母和分子都得到之后就可以用 FGLS 估计随机效应估计量了，记为 $\hat{\mathbf{\beta}}_{RE}$



另外，对于随机效应模型，如果假设扰动项服从正态分布，则可以写出样本的似然函数，然后进行极大似然估计



### 非平衡面板

- 不影响固定效应组间估计量，固定效应照常进行
- 也不怎么影响随机效应，只要 GLS 做广义离差变换时为每个个体 $i$ 根据对应的时间长度 $T_i$ 定义 $\theta_i\equiv1-\dfrac{\hat{\sigma}_{\varepsilon}}{(T_i\hat{\sigma}_u^2+\hat{\sigma}^2_{\varepsilon})^{1/2}}$

最大的问题是，那些原来在样本中但后来退出的个体，可能是自选择的，类似于随机试验中的退出情况，从而引致内生问题。如果从非平衡面板中强行提取一个平衡的面板数据，必然会损失样本容量降低估计效率，而且这二次截取还可能进一步加剧人为选择的问题。



### 豪斯曼检验

究竟用固定效应还是随机效应。原假设 $H_0$：$u_i$ 和所有的解释变量 $(\mathbf{x}_{it},\mathbf{z}_i)$ 均不相关，随机效应正确
...
无法控制所有可能的因素，退而求其次要随机分实验组（处理组）和控制组（对照组），典型如医学实验。安慰剂+双盲

在理想的随机试验中，作为目标解释变量的“处理水平” $x_i$ 是完全随机决定的，肯定和扰动项 $\varepsilon_i$ 不相关，从而没有内生性问题，肯定**一致**。进一步的，处理水平和扰动项独立，满足小样本严外生，所以**无偏**。

所以随机试验中，直接进行单变量回归就可以得到一致无偏的结果。但不管遗漏变量的话，估计系数的方差可能会比较大，所以最好还是可以加上去。
...
- 内部有效性问题
  1. 未能完全随机分组：检验方法是用解释变量 $x_i$ 对所有可观测的个体特征进行回归，如果整体显著，那就是不完全随机
  2. 未能完全遵从实验设计：用设计处理水平作为实际处理水平的工具变量**（？）**
  3. 中途退出实验：选择性偏误。但如果中途退出的原因和实验无关，那就还是随机的，仍然没问题
  4. 霍桑效应：对应方法是双盲法（但不一定可行）
  5. 样本太小：精确度不足
- 外部有效性问题
  1. 样本的代表性不足
  2. 小型实验的条件与大规模推广时的现实条件不同
  3. 一般均衡效应：大面积推广时，参与方可能会改变行为
  4. 自我选择效应：实验或许可以随机，但大规模推广就可能出现更多自选择了



### 自然实验/准实验

外生的突发事件，使得当事人仿佛被随机分到了实验组和对照组，**关键是当事人无法进行自我选择去哪一个组**。经济学常用

1. 第一类自然试验：个体的分组或处理水平完全由自然实验决定，直接 OLS 即可得到因果推断处理效应
2. 第二类自然实验：个体的分组或处理水平只是部分地由自然实验所决定，此时应该以自然实验带来的随机变动作为工具变量进行估计       
...
政策效果往往**需要一段时间**之后才能显现出来

比较适合自然实验/准实验（可以不是随机分组的，关键是不能让当事人有自选择；而且如果真能随机试验，那直接一元回归都可以了，不用 DID）。政策发生前后的池化截面数据（伪面板）也可以用

注意：其实这个方法的应用场合也不一定是要求经过一段时间，关键是如何构建和阐释交互项。
...
$$
y_{it}=\beta_0+\beta_1 D_t+\beta_2 G_i+\beta_3D_t G_i+\varepsilon_{it}\quad(i=1,\cdots,n;t=1,2)
$$

其中

- $D_t$ 为实验期虚拟变量，$t=1$ 时 $D_t=0$，$t=2$ 时 $D_t=1$。**注意刻画的是前后两期本身的差异，不管有没有实际实行政策，也会有这个时间趋势**
- $G_i$ 为政策分组虚拟变量，当 $i\in$ 实验组时 $G_i=1$，否则 $x_i=0$。**注意刻画的是分组的差异，不管有没有实际实行政策，也存在这个组间差别**

可以推得：

- 第二期实验组：$\overline{y}_{i2,treat}=\beta_0+\beta_1+\beta_2+\beta_3$
- 第一期实验组：$\overline{y}_{i1,treat}=\beta_0+\beta_2$
- 第二期对照组：$\overline{y}_{i2,control}=\beta_0+\beta_1$
- 第一期对照组：$\overline{y}_{i1,control}=\beta_0$

从而 $\hat{\beta}_3=\Delta\overline{y}_{treat}-\Delta\overline{y}_{control}=(\overline{y}_{i2,treat}-\overline{y}_{i1,treat})-(\overline{y}_{i2,control}-\overline{y}_{i1,control})$



注意，不是只引入交互项，实验期变量和政策分组变量都必须独立进入模型，否则会有遗漏变量问题。**这里也可以用固定效应的形式进入模型**



### 多期 DID

分批在不同时间点实施的政策也是可以的。操作上只要面板控制时间固定效应即可
...
1. 平行趋势检验
2. 动态效果
3. 安慰剂检验：因为同一时间点可能有其他政策发生了
   1. 选择不同的冲击时间点。通常是选目标政策发生之前的，还是同一个受试对象
   2. 选择不同的受到干预的组
   3. 随机分配干预组和冲击时间
...
例子：假设美国 B 州针对 65 岁或以上的老年人 (实验组，old= 1) 引入一项新的医疗保健政策，其他年龄群体不适用。考察此政策对健康状况的影响。

第一种做法是选用 B 州 65 岁以下群体 (old = 0) 作为对照组。由于人的健康状况随时间的变化并不是线性的，而不同年年龄组的个体的健康状况变化的时间趋势也存在差异，这会导致传统 DID 方法的前提条件——共同趋势假设 (Common Trend) 无法得到满足。简言之，**实验组和对照组人群的健康状况随时间的变化趋势不一致**。

第二种做法是用**相邻** A 州的 65 岁或以上的老年人作为对照组。这种问题是忽略了两州老年人的健康状况即便没有新政策也可能有不同的时间趋势，比如两州州情本来就不同。

所以结合两种做法，将时间趋势差异的影响通过计算**相邻**的 A 州 65 岁及以上老年人和年轻群体相对健康情况变化差异来捕捉 (相当于再用一次 DID)。【本质上是用相邻（相似）的两个个体来获取时间趋势，也就是说用所选两个个体的时间趋势一致的假设来代替平行趋势假设】
$$
y_{ijt}=\beta_0+\beta_1B_i+\beta_2time_t+\beta_3old_j+\beta_4B_itime_t+\beta_5B_iold_j+\beta_6time_told_j+\beta_7B_itime_told_j+\varepsilon_{ijt}
$$
...
$$
\begin{align*}
第二期 B 州老人 &\overline{y}_{BO2}=&\beta_0&+\beta_1&+\beta_2&+\beta_3&+\beta_4&+\beta_5&+\beta_6&+\beta_7\\
第一期 B 州老人 &\overline{y}_{BO1}=&\beta_0&+\beta_1&&+\beta_3&&+\beta_5\\
第二期 B 州非老人 &\overline{y}_{BN2}=&\beta_0&+\beta_1&+\beta_2&&+\beta_4\\
第一期 B 州非老人 &\overline{y}_{BN1}=&\beta_0&+\beta_1\\
第二期 A 州老人 &\overline{y}_{AO2}=&\beta_0&&+\beta_2&+\beta_3&&&+\beta_6\\
第一期 A 州老人 &\overline{y}_{AO1}=&\beta_0&&&+\beta_3\\
第二期 A 州非老人 &\overline{y}_{AN2}=&\beta_0&&+\beta_2\\
第一期 A 州非老人 &\overline{y}_{AN1}=&\beta_0\\
\end{align*}
$$
...
从而 $\hat{\beta_7}=[(\overline{y}_{BO2}-\overline{y}_{BO1})-(\overline{y}_{BN2}-\overline{y}_{BN1})]-[(\overline{y}_{AO2}-\overline{y}_{AO1})-(\overline{y}_{AN2}-\overline{y}_{AN1})]$
...
- 使用工具变量时，因为工具变量只是捕捉了内生解释变量中外生的那部分，其变动 $S_k$ （在分母）肯定不如原来的大，从而估计系数的方差变大，从而可能不显著。如果可以的话，多找几个工具变量，是有可能捕捉到更大的变动 $S_k$，从而使得系数显著
- 怎么找工具变量，
  - 自然实验
    - 天气
  - Bartik 交互项构造，但要保证交互项里每一个组成部分都是外生的
  - 外生制度安排
  - 政策变化
  - 滞后项（包括这个，往下的是没得选的选择）
  - 内生变量同一层级的均值，比如县级财政支出找本省其他县的财政支出平均值（但这种平均值往往自己的变化不够大，从而有弱工具变量问题）
  - 历史、地理（面板数据就用不了，固定效应会消掉），但还是可以用 Bartik 法构造



如果要直接用 OLS，必须保证解释变量 $X$ 真是外生的
